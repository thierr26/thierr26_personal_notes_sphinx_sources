...
``make clean html`` command (case of `Sphinx
...
The ``-C`` switch and the ``--git-dir`` option can be combined to work from
outside a working directory that does not contain any ``.git`` directory or
file.
